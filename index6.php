...
<head>
  ...
  <title>recipe sample site6</title>
...
<!--main部分-->
    ...
    <div class="main-wrapper">
                ...
                <?php
                    ...
                    while($row = mysqli_fetch_array($result, MYSQLI_ASSOC)){
                        if($row['recipe_no'] == $rid){
                            echo "<input type='hidden' name='rid' value='".$row['recipe_no']."'/>";
                            echo '<h4>料理名</h4>';
                            echo "<input type='text' name='recipe_name' size='80' value='".$row['recipe_name']."'>";
                            echo '<div class="recepi_food">';
                            echo '<h4>材料</h4>';
                            echo "<textarea name='foodstuff' rows='10' cols='80' >".$row['recipe_food']."</textarea>";
                            echo '<h4>手順</h4>';
                            echo "<textarea name='foodorder' rows='10' cols='80' >".$row['information']."</textarea>";
                            echo '</div>';
                        }
                    }
        ...
        <input type="submit" value="変更内容投稿">    
        </form>
